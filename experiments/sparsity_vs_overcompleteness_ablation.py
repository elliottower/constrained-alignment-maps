...
app = modal.App("sparsity-vs-overcompleteness-ablation", image=image)
...
def run_comparison(model, pairs, all_acts, labels_for_vae, d_model, n_classes,
    ...
    # ---------------------------------------------------------------
    # ABLATION GRID: disentangle L1 sparsity from overcompleteness.
    #
    # The published pi-SAE differs from pi-VAE in TWO ways at once:
    #   (a) expansion_factor=8  -> 8x wider causal latent (overcomplete)
    #   (b) l1_coeff=1e-3       -> L1 penalty on the causal latent
    # so the pi-VAE(0.00) -> pi-SAE(1.00) jump cannot be attributed to
    # sparsity alone. This grid varies them independently, holding the
    # label-conditional prior and causal/nuisance split fixed.
    #
    # Also tests l1_coeff=1.0, the value the paper reports (code uses 1e-3).
    # ---------------------------------------------------------------
    # Latent dims: identical to the published run (method_comparison_unified.py).
    z_causal = max(k, 1)
    z_nuisance = max(k * 4, 4)
    hidden = min(256, max(128, d_model))

    EXPANSIONS = [1, 8]
    L1_COEFFS = [0.0, 1e-3, 1.0]

    for exp_f in EXPANSIONS:
        for l1 in L1_COEFFS:
            name = f"exp{exp_f}_l1{l1:g}"
            log_msg(f"  Running {name} ({vae_epochs} epochs, expansion={exp_f}, l1={l1:g})...")
            t0 = time.time()
            m = _build_pi_sae(d_model, z_causal, z_nuisance, hidden, n_classes,
                              device, expansion_factor=exp_f)
            train_vae_family(m, all_acts, labels_for_vae, device,
                             n_epochs=vae_epochs, use_pi_prior=True, l1_coeff=l1)
            r = eval_vae_iia(m, model, eval_data, hook_name, device)
            r["method"] = name
            r["expansion_factor"] = exp_f
            r["l1_coeff"] = l1
...
def main(tasks: list[str] = None, k: int = 1):
    ...
    all_results = {}
    out_dir = f"/results/sparsity_vs_overcompleteness_ablation/k{k}"
    ...
    for task, r in all_results.items():
        methods_str = []
        for m in [f"exp{e}_l1{c:g}" for e in [1, 8] for c in [0.0, 1e-3, 1.0]]:
...
def cli(tasks: str = "addition", k: int = 1):
