...
app = modal.App("multiseed-ksweep", image=image)
...
def run_grokking_task(operation, device, k=1, seed=999, model_bundle=None):
    ...
    if model_bundle is not None:
        model, dataset, labels, test_idx, test_acc = model_bundle
    else:
        model, dataset, labels, test_idx, test_acc = train_grokking_model(
            operation, p, device, n_epochs, seed=seed
    )
...
def main(tasks: list[str] = None, ks: str = "1,2,4,8,16,32", seed: int = 999):
    ...
    for task in tasks:
        t0 = time.time()
        # Train the grokking model ONCE per seed, then sweep k against it.
        bundle = None
        if task in GROK_EPOCHS:
            pp = OP_MODULUS.get(task, P)
            log_msg(f"training {task} once for seed {seed} (reused across k={k_list})")
            bundle = train_grokking_model(task, pp, device,
                                          GROK_EPOCHS.get(task, 25000), seed=seed)
        result = {}
        for k in k_list:
            ...
            if task == "ioi":
                result[f"k{k}"] = run_ioi_task(device, k=k)
    ...
    log_msg(f"\n=== seed {seed} done: tasks={list(all_results)} ks={k_list} ===")

    return all_results


@app.local_entrypoint()
def cli(tasks: str = "addition", ks: str = "1,2,4,8,16,32", seed: int = 999):
    task_list = [t.strip() for t in tasks.split(",")]
    result = main.remote(tasks=task_list, ks=ks, seed=seed)
