...
Squaring and cubing achieve $\IIA = 1.0$ at both $k = 2$ and $k = 4$
without grokking (test losses 4.8--17). Intervention-based equivariance
...
Cubic sum ($a^3 + b^3 \bmod 113$) achieves high equivariance despite
cubing alone showing 0\% (intervention-based, $k{=}4$, $p{=}113$).
...
action, which is a stronger condition than linearity. Intervention-based
equivariance confirms: 0\% across all four primes tested.

Max ($\max(a,b) \bmod p$) achieves 15\% equivariance at $p{=}113$ ($k{=}4$),
rising to 29\% at $p{=}53$. It is piecewise equivariant: when the argmax is
stable under the shift, max behaves like a shifted identity. The partial
equivariance reflects this conditional structure.
...
VAE training, geometric diagnostics, and figure generation is archived at
Zenodo (DOI: \href{https://doi.org/10.5281/zenodo.20819724}{10.5281/zenodo.20819724})
and available at
...
repository and Zenodo archive linked above. Experiments use publicly available
