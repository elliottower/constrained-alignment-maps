...
moduli ($p = 53, 97, 113, 211$) and three model depths (1, 2, 4 layers),
totaling 99 trained configurations. We report four findings.
...
71\% ($p{=}211$), and deeper models grok more readily. Two robust extremes
...
where DAS fails. Unconstrained nonlinear baselines achieve perfect IIA
vacuously via degenerate encoder-decoders; we introduce \emph{intervention
faithfulness} metrics (diversity ratio, distributional fidelity) that expose
this failure mode.
...
$p{=}211$, 10 of 14 do. Two-layer models grok more readily than single-layer
models, with operations like subtraction and absolute difference crossing the
grokking threshold at two layers that they fail to cross at one. Two robust
...
A Structured pi-SAE with end-to-end intervention training recovers nonlinear
causal variables where DAS fails. To confirm this method generalizes beyond
grokking, we validate on six GPT-2 language tasks, achieving strict
IIA~$\geq 0.90$ on five of six at $k{=}1$ (Table~\ref{tab:nlp_results}).
...
\item \textbf{Multi-condition atlas.} We construct an atlas of 14 operations
  $\times$ 4 prime moduli $\times$ 3 model depths (99 configurations),
  ...
  to 71\% ($p{=}211$), deeper models grok more readily, and only the
...
\item \textbf{Structured pi-SAE.} A label-conditional sparse autoencoder
  ...
  of six at $k{=}1$) to confirm generalization beyond grokking.
...
and operation (14 types), yielding 99 trained configurations. Training uses
...
into three classes (Table~\ref{tab:main_results}). The multi-modulus sweep
reveals that this partition is not fixed: grokking depends on the interaction
between operation, prime, and depth (Table~\ref{tab:multimod}).
...
\toprule
Operation & $p{=}53$ & $p{=}97$ & $p{=}113$ & $p{=}211$ & 2L \\
\midrule
\multicolumn{6}{l}{\textit{Robust grokkers (grok at $p \geq 97$)}} \\
Addition         & ---  & \checkmark & \checkmark & \checkmark & \checkmark \\
Multiplication   & ---  & \checkmark & \checkmark & \checkmark & \checkmark \\
Division         & ---  & \checkmark & ---  & \checkmark & \checkmark \\
Bitwise XOR      & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
Sum of squares   & ---  & \checkmark & \checkmark & \checkmark & \checkmark \\
\midrule
\multicolumn{6}{l}{\textit{Difficulty spectrum (prime- and depth-dependent)}} \\
Subtraction      & ---  & ---  & ---  & \checkmark & \checkmark \\
Max              & $\sim$ & $\sim$ & \checkmark & \checkmark & --- \\
Min              & $\sim$ & $\sim$ & \checkmark & \checkmark & --- \\
Abs.\ difference & ---  & ---  & ---  & \checkmark & \checkmark \\
Comp.\ addition  & --- & \checkmark & $\sim$ & ---  & \checkmark$^*$ \\
Power            & ---  & ---  & ---  & \checkmark & --- \\
Affine           & ---  & ---  & ---  & ---  & --- \\
\midrule
\multicolumn{6}{l}{\textit{Robust non-grokkers}} \\
Squaring         & ---  & ---  & ---  & ---  & --- \\
Cubing           & ---  & ---  & ---  & ---  & --- \\
\midrule
\multicolumn{6}{l}{\textit{Aggregate grokking rate}} \\
& 1/13 & 6/14 & 6/14 & 10/14 & \\
& (8\%) & (43\%) & (43\%) & (71\%) & \\
\bottomrule
\end{tabular}
\caption{Grokking across primes and depth (1-layer unless noted).
\checkmark = grokked (test CE $< 0.1$); $\sim$ = near-grokked
(acc $> 0.97$ but CE $> 0.1$); --- = did not grok.
2L = 2-layer model at $p{=}113$.
$^*$Comp.\ addition groks at $p{=}97$ (1L and 2L) but not $p{=}113$ or
$p{=}211$ at seed 42---consistent with stochastic classification.
Grokking rate scales monotonically with prime (dataset size $\propto p^2$).}
\label{tab:multimod}
\end{table}
...
Second, deeper models grok more readily. At $p{=}113$ with 2 layers,
subtraction, absolute difference, and division all grok, whereas none of
these grok with 1 layer at the same prime. The additional representational
capacity lowers the grokking threshold, expanding the ``always'' class.
...
At $p{=}113$, power ($a^b \bmod 113$) grokked at seed 137 (test loss 0.088,
equivariance 96.5\%) but not at seed 42 (test loss 1.14, equivariance 66.5\%).
Composite addition ($a + b \bmod 91$) grokked at $p{=}97$ but not at
$p{=}113$ or $p{=}211$ (at seed 42)---the same operation exhibits different
grokking behavior across primes, confirming that the boundary is genuinely
...
but the diversity ratio $\rho \approx 0$ (vs.\ $\rho \approx 0.91$ for the
...
solutions are equally valid. The model's computation defines a large
equivalence class on $\Gr(k,d)$: many subspaces carry the same causal
information, analogous to the rotational degeneracy in attention circuits
where $W_Q \to W_Q R$, $W_K \to W_K R$ leaves $W_Q W_K^\top$ invariant.
...
transformers ($d_\text{model} = 128$). Whether the grokking--Grassmannian
link extends to pretrained language models is an open question, though the
$k$-sweep and equivariance diagnostics apply regardless. The Structured
pi-SAE introduces a model-selection problem (architecture, $\beta$, $\alpha$)
that DAS avoids. The stochastic grokking finding rests on two seeds per
operation at $p{=}113$; multi-seed experiments (10+ seeds) across primes are
needed to estimate grokking probabilities precisely. The equivariance metric
in the multi-modulus sweep had a measurement bug (tolerance too tight for
larger primes); grokking status and IIA are valid, but equivariance values
across primes require re-measurement. The SVD vs.\ DAS comparison is not
...
the Structured pi-SAE's combination of ELBO, label-conditional prior, and
$L_1$ sparsity prevents this failure mode while generalizing to standard
GPT-2 benchmarks (strict IIA $\geq 0.90$ on 5/6 tasks).
