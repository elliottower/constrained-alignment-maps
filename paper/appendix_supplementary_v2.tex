...
collapse; the label-conditional structured VAE values ($\rho \approx 0.89$) confirm
...
Table~\ref{tab:hp_grokking} lists the grokking training hyperparameters.
...
Table~\ref{tab:hp_das} lists the DAS fitting hyperparameters.
...
Table~\ref{tab:hp_pisae} lists the label-conditional structured VAE hyperparameters.
...
Causal latent & expansion $8\times$ \\
Prior & $p(z_\text{causal} \mid y) = \mathcal{N}(\mu_y, \sigma_y^2 I)$ \\
$\beta$ (KL weight) & 1.0 \\
$\alpha$ (classification weight) & 10.0 \\
$\lambda$ ($L_1$ coefficient) & $10^{-3}$ (set in code; ablated, no effect) \\
...
\bottomrule
\end{tabular}
\caption{label-conditional structured VAE hyperparameters. The E2E phase is used for
...
Table~\ref{tab:hp_nldas} lists the NL-DAS hyperparameters.
...
\bottomrule
\end{tabular}
\caption{NL-DAS hyperparameters. Architecture matches the label-conditional VAE encoder
...
\section{Structured VAE ablation details}
\label{app:ablation}

The $2 \times 2$ ablation (Table~\ref{tab:ablation}) tests whether both
components of the label-conditional structured VAE---the label-conditional prior and the
...
\vspace{1em}
Table~\ref{tab:ablation_full} reports the full ablation grid.
...
& \multicolumn{2}{c|}{\textbf{label-conditional VAE}} & \multicolumn{2}{c}{\textbf{Str.\ label-conditional VAE}} \\
...
direction. The label-conditional VAE achieves IIA $\approx 0$ despite correct prior direction
...
Five controls validate that the label-conditional structured VAE's success requires
correct supervision and the causal-nuisance partition, ruling out
trivial explanations:

\vspace{0.5em}
Table~\ref{tab:controls} reports every control.
...
\midrule
Random-label label-conditional VAE    & $\leq 0.02$  & Labels carry no causal signal \\
Reconstruction-only ($\alpha{=}0$) & $\leq 0.005$ & No supervision to pin direction \\
Untrained label-conditional VAE       & 0.000        & Architecture alone is insufficient \\
...
All controls achieve near-zero IIA, confirming that the label-conditional structured VAE's
...
\citet{manir2026systematic} independently find that the effect of activation
...
\section{The Grassmannian manifold}
...
\section{Identifiability of the causal latent factor}
\label{app:of}
...
reparameterization? The label-conditional structured VAE architecture is designed to
...
\section{Continuous distributional metrics}
\label{app:continuous_metrics}

These metrics are defined for completeness and are reported in the
released result files; the main text uses the diversity ratio only.
