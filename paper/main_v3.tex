...
subspace identity is degenerate---10 seeds produce orthogonal subspaces
...
causal variables where DAS fails, achieving strict IIA~$\geq 0.90$ on five of
six GPT-2 language tasks at $k{=}1$ (Table~\ref{tab:nlp_results}).
...
uses the standard grokking setup: 30\% train split, AdamW with learning rate
$10^{-3}$, weight decay 1.0, full-batch (all $0.3 \cdot p^2$ training pairs
per step), and 15{,}000--80{,}000 epochs depending on the operation. We
classify a model as \emph{grokked} if its final test cross-entropy is below 0.1.
...
stochastic operation) are needed to estimate grokking probabilities and
confirm the partition boundaries.
...
Table~\ref{tab:nlp_results} shows that Structured pi-SAE with E2E training
dominates DAS on all six GPT-2 tasks at $k{=}1$. On five of six tasks, E2E
achieves strict IIA $\geq 0.90$; on IOI, gender bias, greater-than, and SVA
it reaches 1.00. The sole exception is capitals (fact retrieval), where
E2E achieves 0.51 standard / 0.26 strict IIA---still $4\times$ better than
DAS (0.12)---but the dataset contains 182 country--capital classes with only
190 examples ($\approx$1 per class), severely limiting classifier training.
This task originates from the RAVEL benchmark \citep{huang2024ravel} and
appears in faithfulness evaluations \citep{prakash2024mib}; the low absolute
IIA reflects a data limitation rather than a method failure.

On hypernymy, E2E achieves IIA~=~0.97 at $k{=}1$---$14\times$ better than
DAS (0.07). Under the strict criterion (source token must beat all 50{,}000
vocabulary tokens), IIA remains 0.90. Note that DAS's strict IIA on SVA is
exactly 0.00 despite a standard IIA of 0.92, revealing that DAS shifts
probability mass toward the correct token without ever making it the argmax.
...
problem (architecture, $\beta$, $\alpha$) that DAS avoids. The stochastic
grokking finding rests on two seeds per operation; multi-seed experiments
(10+ seeds) are needed to estimate grokking probabilities and confirm the
partition boundaries. The connection between the Structured pi-SAE latent
space and the Fourier representation structure identified by
\citet{nanda2023progress} deserves further theoretical analysis. The SVD
vs.\ DAS comparison is not dimension-matched ($k = 32$ vs.\ $k = 8$);
matched-dimension experiments are needed for fair comparison. The GPT-2
evaluation compares methods at different capacity levels (linear DAS vs.\
nonlinear pi-SAE with E2E training); capacity-controlled baselines would
...
on GPT-2 hypernymy where DAS gets 0.07.

Third, continuous distributional metrics (KL, JS, normalized logit difference)
should complement binary IIA. Binary IIA saturates too easily and hides
important quality differences between methods.

Fourth, grokking's creation of causal structure is more nuanced than previously
understood. Nonlinearity is necessary but attention is not (DMF + ReLU groks
18$\times$ faster). Fourier structure \emph{follows} generalization rather than
driving it, and the gradient discovers this structure $\sim$10{,}000 epochs
before the weights crystallize. The causal subspace itself is degenerate---10
seeds produce orthogonal subspaces that all support equally valid causal
interventions---suggesting that \emph{functional} diagnostics (equivariance,
intervention faithfulness) are more robust than \emph{structural} diagnostics
