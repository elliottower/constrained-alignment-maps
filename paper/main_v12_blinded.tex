...
% implemented in experiments/random_network_control.py:run_das (proj @ (src - base), added to h_b)
...
\label{eq:vae_loss}
% implemented in experiments/random_network_control.py:train_vae_family (kl_c, kl_n, ce, recon)
\end{equation}
...
% implemented in experiments/random_network_control.py:run_nldas (featurizer / proj / inv_featurizer)
\label{eq:nldas}
\end{equation}
...
% implemented in experiments/random_network_control.py:compute_diversity_ratio
% MISMATCH: code computes a global std ratio with no E_{y_s} grouping. See Amendment 2.
\end{equation}
...
\subsection{label-conditional structured VAE recovers nonlinear causal variables}
...
and the label-conditional structured VAE provides a constructive resolution: the ELBO's
...
\TODO{RUNNING. \citet{sutter2025nonlinear} report near-100\% IIA for
unconstrained maps on randomly initialised models, which is the most direct test
of whether constraints matter. Both arms launched: GPT-2 with pretrained weights
and with every parameter re-initialised, IOI, otherwise identical. On the random
arm the counterfactual target is set by the task rather than the model's own
outputs, since a random model is correct on nothing. If the label-conditional
structured VAE also scores high there, \S\ref{sec:nldas_vacuous} does not
support the reading given and this paper needs rewriting around that.}
...
\subsection{Four diagnostics, four failure modes}
